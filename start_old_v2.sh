...
if [ "$MODE" = "docker" ] || [ "$MODE" = "d" ]; then
  ...
  # Determine the API URL for the frontend build
  # NEXT_PUBLIC_API_URL is baked into the JS bundle at build time.
  # It MUST be reachable from the user's browser, not from inside Docker.
  # Default: localhost:8000 (works when accessing from the same machine)
  # Override: NEXT_PUBLIC_API_URL=http://192.168.1.50:8000 ./start.sh docker
  if [ -z "${NEXT_PUBLIC_API_URL:-}" ]; then
    export NEXT_PUBLIC_API_URL="http://localhost:8000"
  fi
  info "Frontend will connect browser → ${NEXT_PUBLIC_API_URL}"

  # Pull base images first with retries to avoid mid-build CANCELED errors
  ...
  echo ""
  echo -e "  ${YELLOW}Note:${NC} If accessing from another machine on your network,"
  echo -e "  rebuild with your host IP:"
  echo -e "    ${YELLOW}NEXT_PUBLIC_API_URL=http://<your-ip>:8000 ./start.sh docker${NC}"
  echo ""
